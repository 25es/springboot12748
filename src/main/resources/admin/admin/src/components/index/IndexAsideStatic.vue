  ...
  <el-aside class="index-aside" height="100vh" width="210px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item"
           style="height:100%;broder:0;background-color:#F5F5F5">
        ...
        <el-menu mode="horizontal" :unique-opened="true" class="el-menu-demo" style="height:100%;"
                 background-color="#F5F5F5" default-active="0">
          <el-menu-item index="0" :style="menulistBorderBottom" style="background-color: #333131;color: #FFFFFF" @click="menuHandler('')"><i v-if="true"
          ...
          <el-submenu style="width:138px" :style="menulistBorderBottom"  v-for=" (menu,index) in item.backMenu"
                      :key="menu.menu" :index="index+2+''" >
            <template slot="title">
  ...
  methods: {
    ...
    menuHandler(name) {
      ...
      document.querySelectorAll('.menulist-item>.el-menu--horizontal>.el-submenu>.el-submenu__title').forEach(el => {
        ...
        el.style.backgroundColor= "#333131"
      })
    },
    // 菜单
